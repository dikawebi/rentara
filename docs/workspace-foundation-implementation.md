...
**Status:** `RENTARA-FEAT-002` foundation, `RENTARA-FEAT-003` application shell, `RENTARA-FEAT-004` audit baseline, `RENTARA-FEAT-005` local demo seed, `RENTARA-FEAT-006` property management baseline, `RENTARA-FEAT-007` property structure/unit/assignment baseline, `RENTARA-FEAT-008` amenity/media baseline, `RENTARA-FEAT-009` basic tenant registry, `RENTARA-FEAT-010` contract/occupancy baseline, `RENTARA-FEAT-011` manual Billing Baseline, and `RENTARA-FEAT-012` Maintenance Ticket Workflow Baseline implemented; FEAT-010, FEAT-011, and FEAT-012 are reviewer-approved and QA-passed.

This document describes the current, implemented identity/workspace foundation, application shell, property baseline, property structure/unit/assignment baseline, amenity/private-media baseline, basic tenant registry, internal contract/occupancy baseline, manual Billing Baseline, and internal Maintenance Ticket Workflow Baseline. It does not describe a tenant portal, tenant documents, messaging, marketplace, or invitation functionality.
...
- `audit_logs` (`2026_09_21_000003`, adjusted by `RENTARA-FEAT-002`): `id`, nullable `user_id` and `workspace_id`, `event` string, nullable polymorphic `auditable_type`/`auditable_id` (`nullableMorphs`), nullable JSON `old_values`/`new_values`, nullable `ip_address(45)`, nullable `user_agent` text, and `created_at` only (`AuditLog::UPDATED_AT = null`). Indexes on `[workspace_id, created_at]` and `[user_id, created_at]`.
- After RELEASE-002, `user_id` and `workspace_id` are nullable historical references without foreign keys. Audit history therefore survives deletion of the related user or workspace; the Eloquent relationships are informational, not database-enforced references.
...
- Database enforcement is append-only: update and delete triggers reject mutations to `audit_logs`. The same database-enforced append-only protection applies to maintenance status-history rows. Safe audit scope includes FEAT-010 contract/occupancy events, FEAT-011 invoice/payment events, and FEAT-012 maintenance-ticket events; sensitive tenant fields, secrets, documents, descriptions, and raw request data are excluded.
...
- `media` (`2026_09_22_000011`) stores `workspace_id` and exactly one exclusive private parent: `property_id`, `unit_id`, or `maintenance_ticket_id` (database XOR check). Workspace-safe composite foreign keys enforce that the parent belongs to the same workspace; deleting a workspace/property/unit/maintenance ticket cascades its media rows. A unit media record therefore also belongs to its unit's property.
...
## Manual Billing Baseline (`RENTARA-FEAT-011`)

FEAT-011 implements internal, manual workspace/property-scoped invoices. It is not a tenant-facing billing or payment-processing feature.

### Invoice scope, schema, and status

- Migration `2026_09_23_000015` creates `invoices`, and migration `2026_09_23_000016` adds nullable `paid_date`. Each invoice belongs to one workspace, property, rental contract, contract unit, and one responsible tenant. The graph is server-validated: the contract must be active or expiring, the tenant must be active and attached to that contract, and the property/unit/workspace relationships must agree.
- Invoice terms are an invoice number, billing period, due date, positive integer `amount`, and the workspace's three-letter currency. Invoice number is unique within a workspace; the contract/period combination is also unique within a workspace. Duplicate protection includes soft-deleted history.
- Stored status is only `unpaid` or `paid`. `overdue` is derived, never stored: an unpaid invoice is overdue when its due date is before the current calendar date in the workspace timezone. Paid invoices are never overdue.

### Authorization and payment behavior

- Owners have workspace-wide invoice access. Assigned managers can create, view, and edit unpaid invoice terms for assigned properties. Assigned staff can view and pay invoices for assigned properties but cannot create or edit invoice terms. Super Admins are denied; cross-workspace and unassigned property access is denied server-side.
- A payment is one full manual settlement. It records a required calendar `paid_date` exactly as submitted, plus one controlled method: `cash`, `bank_transfer`, or `other`. Optional reference and notes are stored as payment metadata, not payment proof. Repeating the same payment is idempotent; a conflicting repeat is rejected. Paid invoice terms cannot be edited, and there is no invoice destroy route or paid-bill deletion path.

### Audit and exclusions

- Create, unpaid-term update, and payment events use `invoice.created`, `invoice.updated`, and `invoice.payment_recorded`. Critical invoice writes and audit rows are transaction-coupled; safe audit values contain bounded financial identifiers/status, dates, amount, currency, invoice number, and controlled payment method, excluding payment reference/notes and other raw request data. Parent deletion is restricted while invoice history exists.
- This baseline has no gateway, partial payments, payment proof, recurring billing, reminders, PDF generation, invoice items, penalties, utilities billing, tenant portal, or audit viewer/API.

## Maintenance Ticket Workflow Baseline (`RENTARA-FEAT-012`)

FEAT-012 implements internal, workspace/property-scoped maintenance tickets. Tickets are linked to an active property and unit and may reference an active tenant on that unit. This is an operational workflow, not a tenant-facing support or billing feature.

### Schema, lifecycle, and integrity

- Migration `2026_09_23_000017` creates `maintenance_tickets` and `maintenance_ticket_status_histories`. Tickets store priority, title/description, optional estimated and actual integer costs, optional `charged_to` (`owner` or `tenant`), assignee, submitter, waiting/rejection reasons, and `resolved_at`. Costs and charge destination are recorded metadata only; no invoice or payment is created.
- The statuses are `submitted`, `reviewed`, `assigned`, `in_progress`, `waiting`, `resolved`, `closed`, and `rejected`. The implemented transitions are submitted→reviewed/rejected, reviewed→assigned/rejected, assigned→in_progress/rejected, in_progress→waiting/resolved/rejected, waiting→in_progress/resolved, and resolved→closed. `closed` and `rejected` are terminal.
- Status changes must use the service transition path. Each change appends a history row with actor and optional reason; database guards reject raw status updates and history update/delete operations. Ticket lifecycle and audit writes are transaction-coupled and safe audit values exclude title, description, and raw request data.
- Parent graph validation requires the property and unit to remain in the same workspace/property scope, the property to be active and non-deleted, the unit to be active and non-deleted, and any tenant to be active, non-deleted, and assigned to the selected unit. `charged_to=tenant` requires an active ticket tenant.

### Authorization and operational boundaries

- Owners have workspace-wide ticket access. Assigned managers can view, edit, assign, and transition tickets for their assigned properties. Staff can create tickets for assigned properties, view assigned-property tickets, and edit only their own `submitted` ticket's priority, title, and description. An assigned staff member may perform the operational transitions available from `assigned`, `in_progress`, or `waiting`; staff cannot assign tickets or edit ticket graph, cost, or charge fields.
- Assignment is owner/assigned-manager controlled and may target only an active manager or staff member who is assigned to the ticket property. Staff submission and assignment are therefore bounded by the property assignment and workspace membership checks. Super Admins are denied.
- Ticket photos use the private local media model: the ticket is an exclusive media parent, workspace/property composite scope is enforced, and existing private upload/stream authorization, MIME/size checks, UUID paths, and `nosniff` behavior apply. There is no public ticket-photo endpoint.
- Only owners may soft-delete or restore terminal (`closed`/`rejected`) tickets. Restore requires the active parent graph and any assignee to remain eligible; parent deletion is restricted by ticket history and ticket media foreign keys. There is no force delete.

### Exclusions

FEAT-012 does not implement a tenant portal, messaging/comments/chat, notifications, vendors or SLA management, reporting, billing, payment processing, or an audit viewer/API.
...
There is no tenant shell or tenant portal, tenant contracts/documents, messaging/comments/chat, marketplace, dashboard metrics, notification delivery or inbox, interactive workspace-switch UI, gateway/partial/recurring billing, platform review workflow, ownership transfer, co-owner model, or invitation workflow in this scope. Property scope is the FEAT-006 baseline plus the FEAT-007 structure/unit/assignment and FEAT-008 amenity/private-media baselines above: no public media, image processing, cloud storage, trashed listing, force delete, or property/amenity/media audit events. Maintenance tickets likewise have no vendors/SLA, reporting, billing/payment, notifications, or comments/chat. The shell's dashboard counts remain placeholders. The tenant registry is private and internal; it does not include tenant billing, messaging, contracts/documents, or audit functionality. The demo seed likewise contains no tenant demo account or invitation data.
...
The migrations add the identity columns and create `workspaces` and `workspace_members`; later migrations create the property, structure/unit, assignment, amenity, pivot, media, tenant, rental contract, contract-tenant, check-in, check-out, invoice, maintenance-ticket, and ticket-history tables, with `2026_09_23_000016` adding invoice `paid_date`, `2026_09_23_000018` adding ticket media as an exclusive private-media parent, and `2026_09_23_000019` protecting ticket status/history mutations. The tenant migration does not backfill tenant records or assign existing tenants to workspaces/units, the FEAT-010 contract migrations do not backfill contracts, tenant attachments, occupancy, check-in, or check-out history, FEAT-011 does not backfill invoices or payment history, and FEAT-012 does not backfill tickets, status history, or ticket media. These migrations do not create a workspace or membership for pre-existing users, assign Super Admins, backfill amenities/media, or repair/delete legacy data. Before deploying to an existing environment, back up the database, rehearse all migrations on representative data, verify local private-disk storage permissions, and decide which existing active users require a workspace and active membership. Until valid membership data exists, a verified non-Super-Admin user receives the no-active-workspace response on workspace routes. Run production migrations only through the reviewed deployment process (for example, `php artisan migrate --force`).
...
FEAT-010 reviewer approval and QA PASS were recorded. Verification passed: `php artisan test` reported **168 tests passed, 839 assertions**. Coverage includes contract lifecycle, occupancy/capacity and overlap rules, check-in/out, deposit settlement, authorization, safe transaction-coupled audit behavior, and historical retention boundaries.

FEAT-011 reviewer approval and QA PASS were recorded. The completed verification run passed: `php artisan test` reported **187 tests passed, 934 assertions**; `npm run build` completed successfully. Coverage includes workspace/property/contract/unit/tenant graph validation, integer currency amounts and duplicate boundaries, derived workspace-timezone overdue status, owner/assigned manager/staff authorization, staff view/pay-only behavior, controlled payment methods, calendar-date payment storage, idempotent/conflicting repeat payment handling, transaction-coupled financial audit safety, restrictive invoice history, and the documented billing exclusions.

FEAT-012 reviewer approval and QA PASS were recorded. The completed verification run passed: `php artisan test` reported **209 tests passed, 1005 assertions**; `npm run build` completed successfully. Coverage includes workspace/property/unit/tenant graph validation, lifecycle transition rules, immutable status history and raw-update guards, safe transaction-coupled audit behavior, owner/assigned manager/staff boundaries including staff submission and assigned operational transitions, assignment eligibility, private ticket-photo media security, soft delete/restore parent guards, migration rollback protection, and the documented maintenance-ticket exclusions.

The current RELEASE-002 verification aggregate passed: `php artisan test` reported **214 tests passed, 1029 assertions**. Driver/static migration paths for SQLite, MySQL, and MariaDB boundaries were verified; no live MySQL/MariaDB run is claimed. Live-engine rehearsal remains a prerequisite before production deployment. PostgreSQL is not production-supported.
