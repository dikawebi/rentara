...
- The operational dashboard is at `/app/dashboard` (within the guarded `/app` route group); it displays current workspace context and static placeholder counts, links to property management, and indicates that property and unit management are available while tenant and billing functionality is deferred. Workspace settings and member-management navigation remain policy/context-dependent existing foundation functionality.
