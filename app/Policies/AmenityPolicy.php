<?php

namespace App\Policies;

use App\Enums\PlatformRole;
use App\Enums\UserStatus;
use App\Enums\WorkspaceMemberRole;
use App\Models\Amenity;
use App\Models\User;
use App\Models\Workspace;

class AmenityPolicy
{
    public function viewAny(User $user, Workspace $workspace): bool
    {
        return $this->role($user, $workspace) !== null;
    }

    public function view(User $user, Amenity $amenity): bool
    {
        return $this->role($user, $amenity->workspace) !== null;
    }

    public function create(User $user, Workspace $workspace): bool
    {
        return in_array($this->role($user, $workspace), [WorkspaceMemberRole::Owner, WorkspaceMemberRole::Manager], true);
    }

    public function update(User $user, Amenity $amenity): bool
    {
        return in_array($this->role($user, $amenity->workspace), [WorkspaceMemberRole::Owner, WorkspaceMemberRole::Manager], true);
    }

    public function delete(User $user, Amenity $amenity): bool
    {
        return $this->update($user, $amenity);
    }

    public function restore(User $user, Amenity $amenity): bool
    {
        return $this->update($user, $amenity);
    }

    private function role(User $user, Workspace $workspace): ?WorkspaceMemberRole
    {
        if ($user->platform_role === PlatformRole::SuperAdmin) return null;
        return $workspace->members()->where('user_id', $user->id)->where('status', UserStatus::Active->value)->first()?->role;
    }
}
